...
const openFavoritesButton = document.getElementById('openFavorites');
const transferWalletIconWrap = document.getElementById('transferWalletIconWrap');
const transferWalletIconImg = document.getElementById('transferWalletIconImg');
...
openFavoritesButton.addEventListener('click', () => {
  if (openFavoritesButton.disabled) return;
  renderFavoritesList();
...
function setRetryPaymentRestrictions(enabled, originalAmount = '') {
  ...
  openFavoritesButton.disabled = Boolean(enabled);
  ...
  openFavoritesButton.classList.toggle('locked-action', Boolean(enabled));
  phoneInput.classList.toggle('field-locked', Boolean(enabled));
}


function updateTransferWalletIcon(show = false) {
  const shouldShow = show && validatePhone(normalizePhone(phoneInput.value)) && validateAmount(normalizeAmount(amountInput.value));
  if (!transferWalletIconWrap || !transferWalletIconImg) return;
  transferWalletIconImg.src = selectedWallet === 'jawwal' ? 'icons/jawwalpay-logo-clean.png' : 'icons/palpay-logo-clean.png';
  transferWalletIconWrap.hidden = !shouldShow;
}
